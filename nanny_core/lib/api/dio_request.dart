...
class DioRequest {
    ...
    dio.interceptors.add(
      InterceptorsWrapper(
        onRequest: (options, handler) {
          Logger().d("Sending request to ${options.path} with data:\n${options.data.toString()}");
          
          handler.next(options);
        },
        onResponse: (response, handler) {
          Logger().d("Got response from ${response.requestOptions.path} with data:\n${response.data.toString()}");
    ...
    tokenReloader = Timer.periodic(
      ...
      (_) async {
        ...
        if(reload.response != null) {
          ...
          if(loginData == null) throw Exception("Unhandled token reload error! How did you got here?");
  ...
  static void deleteToken() => dio.options.headers.removeWhere((key, value) => key == "Authorization");

  static Future<bool> handleRequest(BuildContext context, Future<ApiResponse> request) async {
  ...
  static Future<RequestResult<T>> handle<T>(BuildContext context, Future<ApiResponse<T>> request) async {
